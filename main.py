...
def main_origin(
    ...
    from latent_ode_origin import LatentODE
    ...
    @eqx.filter_jit
    def model_eval(model, ts_history_test_i, observation_history_test_i, ts_forward_test_i,
                   observation_forward_test_i, key_i):
        batch_size, _ = ts_history_test_i.shape
        key_i = jrandom.split(key_i, batch_size)
        pred_y = jax.vmap(model.sample)(ts_history_test_i, observation_history_test_i, ts_forward_test_i, key=key_i)
    ...
    axs[0].set_ylabel("x")
    axs = iter(axs)
    iter_train_dataloader = dataloader((ts_train, ys_train), batch_size, key=loader_key)
    iter_test_dataloader = dataloader((ts_history_test, ts_forward_test, observation_history_test,
                                       observation_forward_test), batch_size, key=loader_key)
    for epoch in range(epoch_num):
        for step, (ts_i, ys_i) in enumerate(iter_train_dataloader):
            if ts_i is None:
                break
            start = time.time()
        ...
        if (epoch % save_every) == 0 or epoch == epoch_num - 1:
            sum_rmse = 0
            sum_bs = 0
            for (ts_history_test_i, ts_forward_test_i, observation_history_test_i,
                ...
                pred_y, rmse, batch_size = model_eval(model, ts_history_test_i, observation_history_test_i,
                                                      ts_forward_test_i, observation_forward_test_i, sample_key)
...
def main_controlled(
    ...
    from latent_ode_controlled import LatentODE
